...
public abstract class WizardApiController : ModularWizardApiBaseController
{
    ...
    protected WizardApiController(IModularWizardFactoryRepository repository, IModularWizardFactory wizardFactory, IModularContentFieldRepository fieldRepository, IModularContentTextRepository languageRepository, IModularWizardAdminSettings settings) : base(repository, wizardFactory)
    {
    ...
    /// TODO Edit XML Comment Template for Data
    [HttpPost("SaveNode")]
    public virtual object SaveNode(ModularWizardFieldUpdateData o)
    {
	    var auth = TestAuthDenial(); // Check to see if user has permissions. (Method returns null if authorized, or an object if not authorized)
	    if (auth != null) return auth;
	    if ((auth = TestAuthDenial(_settings.CanReadField, _settings)) != null) return auth;
    ...
    public virtual object LocalizationDtList([FromForm] ModularContentTextDataTablesInput o)
    {
	    var auth = TestAuthDenial(); // Check to see if user has permissions. (Method returns null if authorized, or an object if not authorized)
	    if (auth != null) return auth;
        if ((auth = TestAuthDenial(_settings.CanReadField, _settings)) != null) return auth;
    ...
    public virtual object ErrorMessageDtList([FromForm] ModularContentTextDataTablesInput o)
    {
	    var auth = TestAuthDenial(); // Check to see if user has permissions. (Method returns null if authorized, or an object if not authorized)
	    if (auth != null) return auth;
        if ((auth = TestAuthDenial(_settings.CanReadField, _settings)) != null) return auth;
    ...
    protected virtual IModularWizardContentField InitNewWizardField(IModularWizardContentField o)
    {
        switch (o.FieldType)
        {
            ...
            case nameof(WizardFieldTypes.FORM):
                return new ModularWizardContentField(new
                {
                    o.Id,
                    o.GroupKey,
                    o.FieldKey,
    ...
    public class WizardField : ModularWizardComponentField
    {
    ...
    public class WizardInputModel : WizardField, IModularWizardModelBase
    {
        [JsonProperty(NullValueHandling = NullValueHandling.Ignore, NamingStrategyType = typeof(DefaultNamingStrategy))]
